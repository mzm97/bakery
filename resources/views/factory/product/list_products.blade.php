                        ...
                        <table class="table table-striped table-bordered" id="table1" width="100%">
                            ...
                            @foreach($product as $pro)
                                <tr>
                                    ...
                                    <td>{{ $pro->estimated_price }}</td>
                                    <td>
                                        <a href="list_quantity_product/{{ $pro->product_id }}">{{ $pro->existent_quantity }}</a>
                                    </td>
                                    <td>{{ $pro->estimated_price * $pro->existent_quantity }}</td>
                                    <td>{{ $pro->more_info }}</td>
                                    <td>
                                        <a href="edit_product/{{ $pro->product_id }}"><i class="livicon" data-name="edit" data-size="18" data-c="#428bca" data-hc="#428bca" data-loop="true"></i></a>
                                        <a href="#" role="button" onclick="gettingId({{ $pro->product_id }})" class="confirm-delete" data-id="2" data-toggle="modal"><i class="livicon" data-name="remove-alt" data-size="18" data-c="#f56954" data-hc="#f56954" data-loop="true"></i></a>
                                        <a href="add_quantity_product/{{ $pro->product_id }}"><span>اضافه کردن</span></a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
                    ...
                    <div id="myModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-sm modal-notify modal-danger">
                            ...
                            <div class="modal-content text-center">
                                <form class="text-center" action="{{ url('admin\delete_product') }}" method="post">
                                    {{ csrf_field() }}
                                    <div class="modal-header d-flex justify-content-center">
                                        <h4 class="">آیا مطمئین هستید که حذف گردد؟</h4>
                                        <input type="hidden" name="product_id" id="setVal">
                                    </div>
                                    <!--Body-->
