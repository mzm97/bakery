                        ...
                        <table class="table table-striped table-bordered" id="table1" width="100%">
                            ...
                            @foreach($rawMaterial as $raw)
                                <tr>
                                    ...
                                    <td>{{ $raw->estimated_price }}</td>
                                    <td>
                                        <a href="list_use_raw_material/{{ $raw->raw_material_id }}">{{ $raw->existent_quantity }}</a>
                                    </td>
                                    <td>{{ $raw->unit }}</td>
                                    <td>{{ $raw->estimated_price * $raw->existent_quantity }}</td>
                                    <td>{{ $raw->more_info }}</td>
                                    <td>
                                        <a href="edit_raw_material/{{ $raw->raw_material_id }}"><i class="livicon" data-name="edit" data-size="18" data-c="#428bca" data-hc="#428bca" data-loop="true"></i></a>
                                        <a href="#" role="button" onclick="gettingId({{ $raw->raw_material_id }})" class="confirm-delete" data-id="2" data-toggle="modal"><i class="livicon" data-name="remove-alt" data-size="18" data-c="#f56954" data-hc="#f56954" data-loop="true"></i></a>
                                        <a href="use_raw_material/{{ $raw->raw_material_id }}"><span>استفاده</span></a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
                    ...
                    <div id="myModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-sm modal-notify modal-danger">
                            ...
                            <div class="modal-content text-center">
                                <form class="text-center" action="{{ url('admin\delete_raw_material') }}" method="post">
                                    {{ csrf_field() }}
                                    <div class="modal-header d-flex justify-content-center">
                                        <h4 class="">آیا مطمئین هستید که حذف گردد؟</h4>
                                        <input type="hidden" name="raw_material_id" id="setVal">
                                    </div>
                                    <!--Body-->
